...
/**
 * {@code CamelForkedDebuggerThread} is a fork of the class {@code ForkedDebuggerThread} that is
 * needed to be able to have {@code CamelExternalSystemTaskDebugRunner} working as expected.
 */
class CamelForkedDebuggerThread extends Thread {
    ...
    CamelForkedDebuggerThread(@NotNull ProcessHandler mainProcessHandler,
        ...
        super("camel external task forked debugger runner");
        setDaemon(true);
        mySocket = socket;
    ...
    public void run() {
        while (!myMainProcessHandler.isProcessTerminated() && !myMainProcessHandler.isProcessTerminating() && !mySocket.isClosed()) {
            try {
                if (CamelExternalSystemTaskDebugRunner.LOG.isDebugEnabled()) {
                    int port = mySocket.getLocalPort();
                    String host = mySocket.getInetAddress().getHostAddress();
                    String productName = ApplicationNamesInfo.getInstance().getFullProductName();
                    CamelExternalSystemTaskDebugRunner.LOG.debug(String.format("%s wait for debug process signal on '%s:%d'", productName, host, port));
                }
                handleForkedProcessSignal(mySocket.accept());
            } catch (EOFException ignored) {
            } catch (SocketException e) {
                CamelExternalSystemTaskDebugRunner.LOG.debug(e);
            } catch (IOException e) {
                CamelExternalSystemTaskDebugRunner.LOG.warn(e);
            }
        }
        try {
            if (!mySocket.isClosed()) {
    ...
    private void handleForkedProcessSignal(Socket accept) throws IOException {
        ...
        if (CamelExternalSystemTaskDebugRunner.LOG.isDebugEnabled()) {
            ...
            CamelExternalSystemTaskDebugRunner.LOG.debug(String.format(logMessage, productName, debuggerId, processName, processParameters));
        }
    ...
    private void attachRemoteDebugger(
        ...
        if (extension != null) {
            ...
            if (myMainRunnableState.isReattachDebugProcess()) {
                ...
                CamelExternalSystemTaskDebugRunner.LOG.warn("Unsupported reattach child debugger process into main process");
            }
    ...
    private void reattachRemoteDebugger(@NotNull RemoteConfiguration runConfiguration, @NotNull Consumer<? super DebugProcess> callback) {
        ...
        if (debugProcess instanceof DebugProcessImpl debugProcessImpl) {
            ...
            ApplicationManager.getApplication().invokeAndWait(() -> debugProcessImpl.reattach(environment, true, () -> callback.accept(debugProcess)));
        }
    }
    ...
    private static void runDebugConfiguration(@NotNull RunnerAndConfigurationSettings runSettings, ProgramRunner.Callback callback) {
        try {
            ...
            ApplicationManager.getApplication().invokeAndWait(() -> ProgramRunnerUtil.executeConfigurationAsync(environment, true, true, callback));
        } catch (ExecutionException e) {
            CamelExternalSystemTaskDebugRunner.LOG.error(e);
        }
    }
    ...
    private static void buildWithRuntimeModuleDir(@NotNull RunnerAndConfigurationSettings runSettings, ExecutionEnvironmentBuilder builder) {
        ...
        if (configuration instanceof UserDataHolder userDataHolder) {
            String moduleDir = userDataHolder.getUserData(RUNTIME_MODULE_DIR_KEY);
            if (moduleDir != null) {
    ...
    private class MyForkedProcessListener extends ProcessAdapter {
        ...
        private void postLink() {
            ...
            if (mainConsoleView != null) {
                ...
                if (debugProcess instanceof DebugProcessImpl debugProcessImpl) {
                    addressDisplayName = "(" + JavaDebuggerBundle.getAddressDisplayName(debugProcessImpl.getConnection()) + ")";
                }
        ...
        private ConsoleViewImpl getMainConsoleView() {
            ...
            if (executionConsole instanceof DataProvider dataProvider) {
                Object consoleView = dataProvider.getData(LangDataKeys.CONSOLE_VIEW.getName());
        ...
        if (debugProcess != null) {
            debugProcess.getManagerThread().invokeCommand(new DebuggerCommand() {
                ...
                public void action() {
                    ...
                    if (virtualMachineProxy instanceof VirtualMachineProxyImpl proxy
                        && proxy.canBeModified()) {
